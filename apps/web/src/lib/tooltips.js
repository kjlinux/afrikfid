...
export const TOOLTIPS = {
  ...
  // --- Campagnes & Triggers ---
  campagne_rfm: "Campagne RFM : message envoyé en masse à un segment précis de clients (ex. Champions, À Risque). Basée sur leur comportement d'achat calculé automatiquement par le moteur RFM.",
  campagne_ciblee: "Campagne ciblée (démographique) : message envoyé à des clients filtrés selon des critères personnalisés — âge, ville, genre, statut fidélité, inactivité, etc. Plus flexible qu'une campagne RFM.",
  trigger: "Trigger automatique : message déclenché automatiquement par un événement client (premier achat, anniversaire, longue absence…). Il s'envoie sans action de votre part dès que la condition est remplie.",
  trigger_bienvenue: "BIENVENUE : envoyé automatiquement au nouveau client lors de sa première inscription au programme de fidélité de votre commerce.",
  trigger_1er_achat: "1ER_ACHAT : envoyé après le tout premier achat d'un client chez vous. Idéal pour remercier et l'inciter à revenir rapidement.",
  trigger_absence: "ABSENCE : envoyé quand un client habituel n'est pas revenu depuis un certain nombre de jours. Permet de le relancer avant qu'il parte définitivement.",
  trigger_alerte_r: "ALERTE_R : envoyé quand le score de Récence (R) d'un client chute — c'est-à-dire qu'il n'a pas acheté depuis trop longtemps. Signal d'alerte précoce avant le churn.",
  trigger_a_risque: "A_RISQUE : envoyé aux clients dont le profil RFM indique un risque élevé de ne plus revenir. Action commerciale urgente recommandée.",
  trigger_win_back: "WIN_BACK : envoyé aux clients inactifs depuis longtemps pour tenter de les récupérer avec une offre forte (-20%, points x3, etc.).",
  trigger_anniversaire: "ANNIVERSAIRE : envoyé automatiquement le jour ou le mois d'anniversaire du client. Fort impact émotionnel, taux d'ouverture élevé.",
  trigger_palier: "PALIER : envoyé quand un client est proche d'atteindre le palier fidélité suivant (ex. passage de LIVE à GOLD). L'incite à faire un achat supplémentaire pour débloquer l'avantage.",
  cooldown: "Cooldown : délai minimum entre deux envois du même trigger au même client. Évite de le sur-solliciter. Exemple : 24h = un seul envoi par jour maximum.",
  canal_whatsapp: "WhatsApp (Lafricamobile) : envoi via le réseau WhatsApp Business. Taux d'ouverture très élevé (>90%). Nécessite un template pré-approuvé ou un message texte libre selon votre configuration.",
  canal_email: "Email : envoi par courriel. Plus adapté aux messages longs ou avec des visuels. Taux d'ouverture plus faible qu'un WhatsApp mais idéal pour des offres détaillées.",
  template_whatsapp: "Template WhatsApp : modèle de message pré-approuvé par Meta (WhatsApp Business). Obligatoire pour initier une conversation. Les variables {client_name}, {merchant_name} sont remplacées automatiquement.",
  message_template: "Message libre : texte envoyé au client. Utilisez {client_name} pour insérer son prénom et {merchant_name} pour le nom de votre commerce. Gardez-le court et percutant.",
  segment_cible: "Segment cible : groupe de clients à qui la campagne ou le trigger sera envoyé. Calculé automatiquement par le moteur RFM selon le comportement d'achat de chaque client.",
  statut_campagne_draft: "Brouillon (draft) : campagne créée mais pas encore envoyée. Cliquez sur 'Lancer' pour l'exécuter.",
  statut_campagne_scheduled: "Planifiée (scheduled) : campagne prête à être lancée. Cliquez sur 'Lancer' pour déclencher l'envoi immédiatement.",
  statut_campagne_running: "En cours (running) : campagne en cours d'envoi. Les messages partent progressivement vers les clients ciblés.",
  statut_campagne_completed: "Terminée (completed) : tous les messages ont été envoyés. Consultez le ratio envoyés / cibles pour évaluer le résultat.",
  statut_campagne_cancelled: "Annulée (cancelled) : campagne interrompue avant d'être terminée.",
  inactivite_jours: "Inactif depuis X jours : filtre ciblant les clients qui n'ont pas acheté chez vous depuis au moins X jours. Utile pour les campagnes de réactivation.",
  audience_estimee: "Audience estimée : nombre de clients de votre base qui correspondent aux critères de ciblage définis. Cliquez sur 'Prévisualiser' pour calculer avant d'envoyer.",
}
