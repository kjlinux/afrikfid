...
const LEVEL_CONFIG = {
  critical: { label: 'Critique', color: 'var(--af-danger)',      bg: 'var(--af-danger-soft)',  border: 'var(--af-danger)' },
  high:     { label: 'Élevé',    color: 'var(--af-warning)',     bg: 'var(--af-warning-soft)', border: 'var(--af-warning)' },
  medium:   { label: 'Modéré',   color: 'var(--af-accent)',      bg: 'var(--af-accent-soft)',  border: 'var(--af-accent)' },
  low:      { label: 'Faible',   color: 'var(--af-text-muted)',  bg: 'var(--af-surface-2)',    border: 'var(--af-border)' },
}
...
function ScoreBar({ score }) {
  ...
  const color = score >= 0.8 ? 'var(--af-danger)' : score >= 0.6 ? 'var(--af-warning)' : score >= 0.3 ? 'var(--af-accent)' : 'var(--af-text-muted)'
  return (
  ...
  if (error?.upgrade) return (
    <div style={{ padding: '28px 32px' }}>
      ...
      <div style={{ background: 'var(--af-surface-2)', border: '1px solid var(--af-border)', borderRadius: 12, padding: '40px 32px', textAlign: 'center' }}>
        <ChartBarIcon style={{ width: 48, height: 48, color: 'var(--af-text-muted)', margin: '0 auto 16px' }} />
        <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--af-text)', marginBottom: 8 }}>Fonctionnalité non incluse dans votre package actuel</div>
        ...
        {upgradableTo.length > 0 && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
            {upgradableTo.map(pkg => (
              <span key={pkg} style={{ padding: '3px 10px', borderRadius: 20, fontSize: 12, fontWeight: 600, background: 'var(--af-surface)', color: 'var(--af-text-muted)', border: '1px solid var(--af-border)' }}>
                {PKG_LABELS[pkg]}
              </span>
            ))}
          </div>
  ...
  return (
    <div style={{ padding: '28px 32px' }}>
      ...
      {error?.msg && !error?.upgrade && (
        <div style={{ background: 'var(--af-danger-soft)', border: '1px solid var(--af-danger)', color: 'var(--af-danger)', borderRadius: 8, padding: '10px 14px', fontSize: 13, marginBottom: 16 }}>{error.msg}</div>
      )}
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
            {[
              ...
              { label: 'Critiques', value: data.summary?.by_level?.critical ?? 0, color: 'var(--af-danger)', sub: 'score ≥ 80%', tip: null },
              { label: 'Élevés', value: data.summary?.by_level?.high ?? 0, color: 'var(--af-warning)', sub: 'score 60-79%', tip: null },
          ...
          {(data.predictions || []).length === 0 ? (
            <div style={{ ...card, textAlign: 'center', padding: '40px 32px' }}>
              <CheckCircleIcon style={{ width: 40, height: 40, color: 'var(--af-success)', margin: '0 auto 12px' }} />
              <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--af-success)', marginBottom: 4 }}>Aucun client à risque détecté</div>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                {(selected.signals || []).map((s, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '6px 0', borderBottom: i < selected.signals.length - 1 ? '1px solid var(--af-surface)' : 'none' }}>
                    <ExclamationTriangleIcon style={{ width: 14, height: 14, color: 'var(--af-warning)', flexShrink: 0, marginTop: 1 }} />
                ...
                <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
                  {[
                    ...
                    <div key={s.label} style={{ flex: 1, background: 'var(--af-surface-3)', border: '1px solid var(--af-border)', borderRadius: 8, padding: '10px 12px', textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: 20, fontWeight: 800, color: s.value >= 4 ? 'var(--af-success)' : s.value >= 3 ? 'var(--af-accent)' : 'var(--af-danger)' }}>{s.value}<span style={{ fontSize: 12, color: 'var(--af-text-muted)' }}>/5</span></div>
                    </div>
                  ))}
                </div>
              )}
              <div style={{ padding: '10px 14px', background: 'var(--af-surface-2)', borderLeft: '4px solid var(--af-accent)', borderRadius: 8, fontSize: 12, color: 'var(--af-text-muted)', display: 'flex', alignItems: 'flex-start', gap: 8 }}>
