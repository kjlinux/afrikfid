  ...
  return (
    <div style={{ padding: '28px 32px' }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: '#f1f5f9' }}>Taux de Change</h1>
        <p style={{ color: '#64748b', fontSize: 13, marginTop: 2 }}>XOF · XAF · KES · EUR — Taux utilisés pour la normalisation des rapports</p>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20 }}>
          <Card title="Taux configurés">
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {rates.map(r => (
                  <tr key={`${r.from_currency}-${r.to_currency}`} style={{ borderTop: '1px solid #334155' }}>
                    <td style={{ padding: '11px 14px', fontSize: 14, fontWeight: 600, color: '#f59e0b' }}>{r.from_currency}</td>
                    <td style={{ padding: '11px 14px', fontSize: 14, fontWeight: 600, color: '#3b82f6' }}>{r.to_currency}</td>
                    <td style={{ padding: '11px 14px', fontSize: 13, color: '#f1f5f9', fontFamily: 'monospace' }}>{r.rate}</td>
                    <td style={{ padding: '11px 14px', fontSize: 11, color: '#64748b' }}>{ZONES[r.from_currency] || '—'}</td>
                    <td style={{ padding: '11px 14px', fontSize: 11, color: '#64748b' }}>{r.updated_at?.split('T')[0]}</td>
                    <td style={{ padding: '11px 14px' }}>
                      <button onClick={() => startEdit(r)}
                        style={{ padding: '3px 10px', background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 6, color: '#f59e0b', cursor: 'pointer', fontSize: 11, fontWeight: 600 }}>
                        Modifier
                      </button>
                    </td>
                  </tr>
          ...
          <Card title={editing ? `Modifier ${editing.from_currency} → ${editing.to_currency}` : 'Sélectionnez un taux'}>
            {editing ? (
              <div>
                <div style={{ background: '#0f172a', borderRadius: 8, padding: 12, marginBottom: 16 }}>
                  <div style={{ fontSize: 11, color: '#64748b', marginBottom: 4 }}>Taux actuel</div>
            ...
            {/* Info */}
            <div style={{ marginTop: 20, background: '#0f172a', borderRadius: 8, padding: 12 }}>
              <div style={{ fontSize: 11, color: '#64748b', fontWeight: 600, marginBottom: 8 }}>ℹ️ UTILISATION</div>
