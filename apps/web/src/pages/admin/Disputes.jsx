    ...
    <Modal open title={`Litige — ${dispute.tx_reference || dispute.id.slice(0, 12)}`} onClose={onClose}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 16 }}>
        ...
        <div>
          ...
          <div style={{ color: 'var(--af-text)', fontSize: 13 }}>{REASON_LABELS[dispute.reason] || dispute.reason}</div>
        </div>
        <div>
          <div style={{ color: 'var(--af-text-muted)', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 }}>Montant disputé</div>
        ...
        <div>
          ...
          <div style={{ color: 'var(--af-text)', fontSize: 13 }}>{dispute.merchant_name || '—'}</div>
        </div>
        <div>
          <div style={{ color: 'var(--af-text-muted)', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 }}>Client</div>
          <div style={{ color: 'var(--af-text)', fontSize: 13 }}>{dispute.client_name || '—'}{dispute.afrikfid_id ? ` (${dispute.afrikfid_id})` : ''}</div>
        </div>
        <div>
          ...
          <div style={{ color: 'var(--af-text)', fontSize: 13 }}>{dispute.initiated_by || '—'}</div>
        </div>
        <div style={{ gridColumn: '1 / -1' }}>
          <div style={{ color: 'var(--af-text-muted)', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 }}>Description</div>
          <div style={{ color: 'var(--af-text-muted)', fontSize: 12, background: 'var(--af-surface-3)', borderRadius: 6, padding: '8px 12px', lineHeight: 1.6 }}>
            {dispute.description || 'Aucune description fournie'}
          </div>
        </div>
        {dispute.resolution_note && (
          <div style={{ gridColumn: '1 / -1' }}>
            <div style={{ color: 'var(--af-text-muted)', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 }}>Note de résolution</div>
            <div style={{ color: 'var(--af-text-muted)', fontSize: 12, background: 'var(--af-surface-3)', borderRadius: 6, padding: '8px 12px', lineHeight: 1.6 }}>
              {dispute.resolution_note}
            </div>
          </div>
      ...
      {detail?.history?.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ color: 'var(--af-text-muted)', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Historique</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {detail.history.map(h => (
              <div key={h.id} style={{ background: 'var(--af-surface-3)', borderRadius: 6, padding: '7px 12px', fontSize: 12, display: 'flex', gap: 8, alignItems: 'baseline' }}>
                <span style={{ color: 'var(--af-border-strong)', flexShrink: 0 }}>{new Date(h.created_at).toLocaleString('fr-FR')}</span>
                <span style={{ color: 'var(--af-text-muted)' }}>{h.action.replace(/_/g, ' ')}</span>
                {h.note && <span style={{ color: 'var(--af-text)' }}>— {h.note}</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      {!isClosed && (
        <div style={{ borderTop: '1px solid var(--af-surface)', paddingTop: 14 }}>
          <div style={{ color: 'var(--af-text-muted)', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10 }}>Mettre à jour</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <textarea
              ...
              rows={3}
              style={{ background: 'var(--af-surface-3)', border: '1px solid var(--af-border)', borderRadius: 6, color: 'var(--af-text)', padding: '8px 12px', fontSize: 13, resize: 'vertical', outline: 'none' }}
            />
        ...
        {loading ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 680 }}>
              ...
              <tbody>
                {disputes.map(d => (
                  <tr key={d.id} style={{ borderBottom: '1px solid var(--af-surface-3)' }}>
                    ...
                    <td style={{ padding: '11px 16px', color: 'var(--af-text)', fontSize: 13 }}>
                      {d.merchant_name || '—'}
                    </td>
                    <td style={{ padding: '11px 16px', color: 'var(--af-text-muted)', fontSize: 12 }}>
